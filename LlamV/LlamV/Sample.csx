...
{
    raw["Active"].Signal(c).ToText(ref result, (x) => $"Ave{x.Item1}\r\nDev{x.Item2}\r\n");
    raw["HOB-L"].Signal(c).ToText(ref result, (x) => $"Ave:{x.Item1}\r\nDev:{x.Item2}\r\n");
    raw["HOB-R"].Signal(c).ToText(ref result, (x) => $"Ave:{x.Item1}\r\nDev:{x.Item2}\r\n");

    raw["Active"].AverageV(c).LineStatus(5).ToText(ref result, (x) =>
        $"Ave:{x.Item1}\r\n"
      + $"Dev:{x.Item2}\r\n"
      + $"Max:{x.Item3}\r\n"
      + $"Min:{x.Item4}\r\n"
    ...
    raw["Active"].AverageH(c).LineStatus(5).ToText(ref result, (x) =>
        $"Ave:{x.Item1}\r\n"
      + $"Dev:{x.Item2}\r\n"
      + $"Max:{x.Item3}\r\n"
      + $"Min:{x.Item4}\r\n"
...
foreach (var c in new string[] { "Gr", "R", "B", "Gb" })
{
    defect["Active"].Count(x => x > 125, c).ToText(ref result, (x) => $"WD:{x}\r\n" );
    defect["Active"].Count(x => x < -125, c).ToText(ref result, (x) => $"BD:{x}\r\n");
}
